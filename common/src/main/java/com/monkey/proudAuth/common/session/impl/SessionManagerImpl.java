...
public final class SessionManagerImpl implements SessionManager {
    ...
    @Override
    public CompletableFuture<Optional<Session>> findValid(UUID uuid, String ip) {
        if (!settings.sessions().enabled()) {
            return CompletableFuture.completedFuture(Optional.empty());
        }
        String resolvedIp = settings.sessions().bindToIp() ? ip : null;
        return storage.findValidSession(uuid, resolvedIp);
    }

    @Override
    public CompletableFuture<Session> create(UUID uuid, String ip) {
        ...
        Session session = new Session(generateToken(), ip, uuid, expiresAt);
        if (!settings.sessions().enabled()) {
